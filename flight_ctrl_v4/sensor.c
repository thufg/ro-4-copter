...
static uint8_t adc_chan_order[3];
...
ISR(ADC_vect)
{
	...
	adc_chan %= 3; // overflow channel count
	...
	if(adc_chan == 0)
	{
		...
		memcpy(adc_res, adc_res_t, sizeof(int32_t) * 3);
		adc_new_cycle++;
	}
}
...
void sens_init()
{
	...
	adc_chan_order[0] = roll_gyro_chan;
	adc_chan_order[1] = pitch_gyro_chan;
	adc_chan_order[2] = yaw_gyro_chan;
...
{
	volatile uint64_t sum[3] = {0,0,0,0,0,0,0,0};
